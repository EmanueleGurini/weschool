...
export default function TableTeacherClass({ students, id, date, subjects, dateSelected }: IClassData) {
  async function handleClose(e: MouseEvent<HTMLButtonElement>, classID: string, status: boolean, date: string): Promise<void> {
    ...
    await supabase.from("attendance").insert({ student_id: idStudent, class_id: classID, status: status, date: date }).select();

    window.location.reload();
  }
  const uniqueStudents = students.filter((student, index, self) => index === self.findIndex((s) => s.fullName === student.fullName));
  return (
              ...
              <tr key={student.id} className="last:rounded-bl-lg last:rounded-br-lg">
                ...
                <td className="px-6 py-3 whitespace-nowrap border-b border-gray-300">
                  <Link
                    ...
                    className="inline-block rounded-lg border-2 border-contrast py-3 px-6 font-sans text-xs font-bold uppercase text-contrast bg-transparent shadow-md transition-all hover:border-contrasthover hover:text-contrasthover focus:opacity-85 active:opacity-85 disabled:pointer-events-none disabled:opacity-50"
                  >
